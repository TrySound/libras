...
`src/storage.ts` contains the application-owned `Storage` implementation and shared snapshot types/validation. Session creates immutable account-configured instances with `new Storage(account)`. `MetadataEngine` receives the selected instance through restore/accept operations and uses `storage.metadata().read()` / `.save(snapshot, valid)`; it retains remote normalization, refresh decisions, staging, and Memory publication. Account access is lazy, captures identity, and shares underlying file caches and write ordering. Storage keeps existing account-scoped paths, atomic newer-snapshot comparisons, and repair-on-write policy. The same configured instance provides `storage.queue().read()` / `.save(record)` to QueueEngine, preserving duplicate occurrence indices and pending-sync markers. Queue saves compare timestamps under the file lock and return whether a write occurred plus the stored record; QueueEngine retains conflict reporting, local/server write tails, checkpoint scheduling, and sync decisions. CoverEngine receives the selected workspace during restoration and uses `storage.artwork()` for catalog reads/updates, image integrity checks, immutable image bytes, and binary/catalog commits. Missing-file repair can return a filtered catalog with an error if the repair cannot be persisted; corrupt catalogs are not silently replaced. Failed, superseded, or losing image writes remove only their new unused file. CoverEngine retains fallback ordering, revalidation policy, Memory publication, and object URL ownership. `storage.audio()` provides the shared cross-account downloads catalog and descriptor-scoped `read`/`save` operations. Its private implementation stays in `src/storage.ts`; `list()` validates saved files, while `entries()` returns the loaded catalog after pending catalog writes. TrackEngine receives the selected workspace during restoration, derives descriptors from its immutable account, and retains scheduling, format selection, Memory publication, and playback object URLs. Audio writes still stream directly to OPFS, use the existing lock order, release unused responses, and preserve legacy-file adoption and complete orphan files.

The feature APIs intentionally differ: metadata returns the winning snapshot, queue returns a write result, artwork separates catalogs from image bytes, and audio exposes a cross-account catalog. Keep these domain distinctions rather than introducing an interchangeable backend interface. Per-account file caches share ordering, and audio's private index avoids re-reading the catalog for every publication. Engine write tails, generation checks, and browser resource ownership serve different purposes from Storage's locks and atomic commits. Small repetitions in account-file initialization do not require a generic store factory.
