...
`Cache.load()` restores library and queue independently. It waits for both attempts, retains each successful result, and reports domain failures through `CacheLoadError`. Queue errors also remain on `queueError`, so a successful library refresh cannot hide them. Restoration never overwrites pending queue edits. There are no legacy queue reads or duplicate queue stores.
