...
- All cache implementation lives in `src/cache.svelte.ts`. Four independent domain queues serialize local I/O plus publication using one small `serial()` helper. The private `Disk` helper shares one lazily computed account namespace across JSON and binary operations, opens directories on demand, and provides locked JSON reads/atomic updates without another scheduling layer. The former `json-store.ts` wrapper is removed. Library parsing checks schema/uniqueness without building indexes; publication builds the relationships once and retains collections plus freshness, not the original snapshot arrays.
- Nonempty corrupt documents are preserved by default; library and queue explicitly permit repair on write. Repair never suppresses permission or byte-read failures. Without Web Locks, failed writes leave shared JSON placeholders untouched to avoid deleting a competing writer's commit; zero-byte placeholders contain no record and are treated as absent on subsequent reads. Binary files have unique names and retain their existing cleanup rules. File formats and lock names are unchanged.
