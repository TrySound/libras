...
`app.svelte` constructs the domain engines, `Network`, `Session`, and transitional `Memory`. UI and resource engines continue reading Memory. Its read-only library getters delegate to the selected account's reactive `Cache` without copying records; direct UI cache access is deferred. Memory retains queue/artwork/download state until those domains migrate. Credentials, network clients, authenticated URLs, object URLs, and workflow state stay outside the cache.
...
| `Cache` | Account-scoped library hydration, validation, atomic persistence, reactive collections, and derived relationship indexes |
...
- **Server snapshots:** MetadataEngine fetches a library and delegates replacement to Cache; it does not index or publish records. QueueEngine still owns its fetch → persist → publish workflow. Failed saves do not publish incoming data. Server timestamp checks and queue occurrence-index matching stay beside their domain workflows. Subsonic-specific normalization stays in Network.
...
`src/cache.svelte.ts` owns the library's runtime and persisted data. Session creates an account cache for startup and a separate candidate cache for each explicit connection, selecting the latter only after persistence and network acceptance. Session calls `Cache.load()` directly and waits for local restoration before attaching network access. MetadataEngine neither loads nor selects caches and receives only a read-only view of the selection. A failed library load is reported without blocking queue restoration or online recovery. Currently only the library is wired to engines and Memory. The cache also has a queue foundation, but QueueEngine still uses the old queue state/store until its integration step. Memory proxies library collections and MetadataEngine proxies their freshness information for existing consumers; neither exposes a writable snapshot.

The queue-cache foundation stores one `{ tracks, index, position }` value at `accounts/<account-hash>/queue.json`, separate from the library. `setQueue()` copies and validates the value, publishes immediately, and schedules a 300 ms debounced save with a five-second maximum wait. `flush()` saves pending edits and returns the revision actually committed; edits made during that write remain dirty. `queueDirty`, `queueRevision`, and `queueError` expose local durability, not server upload eligibility. A newer disk timestamp causes a conflict rather than replacing the optimistic queue or acknowledging a save. Failed background writes remain visible and wait for another edit or explicit flush to retry.

`Cache.load()` restores library and queue independently. It waits for both attempts, retains each successful result, and reports domain failures through `CacheLoadError`. Queue errors also remain on `queueError`, so a successful library refresh cannot hide them. Restoration never overwrites pending queue edits. There are no legacy queue reads in Cache; the existing QueueEngine/store still operate separately until integration.

`src/storage.ts` still contains account-configured queue, artwork, and audio capabilities for domains not yet migrated. Session caches these Storage instances. The old metadata store is removed. Each cache/store acquires files lazily; no interchangeable backend abstraction is needed.

Cache library replacement adopts the winning disk snapshot internally; queue saves return whether a record was written; artwork separates catalog updates from image commits; audio streams responses to disk and maintains its shared catalog internally. Audio access remains scoped to the configured account. Persisted/untrusted reads are validated; already-typed writes are not reparsed. Engines own publication, local write ordering, and browser object URLs. Storage owns file I/O, Web Locks, atomic replacement, and persisted-record validation.
