...
- `website.svelte` owns the landing page, live-demo frame, feature illustrations, setup guide, FAQ, and calls to action. `website.css` extends the shared `src/app.css` design system with website-scoped tokens and `site-*` classes; marketing typography excludes the embedded app. Illustrations use CSS and the shared icon sprite, with no external fonts or image requests. The page adapts to mobile, respects reduced motion, and uses native links and disclosure elements.
